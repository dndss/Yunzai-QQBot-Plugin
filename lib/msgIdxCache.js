...
async function adaptStoredRecord (botOrId, record) {
  ...
  const element = Array.isArray(elements)
    ...
    : undefined
  if (!element?.content) return data
  const fallback = {
    message: parseElementContent(element.content),
    raw_message: element.content,
    sender: {},
    time: data.time,
    group_id: data.group_id,
  }
  data.source = { user_id: "", time: data.time, seq: "", rand: 1, message: element.content }
