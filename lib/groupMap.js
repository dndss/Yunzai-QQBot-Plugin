...
function isQQGroupUin (value) {
  return /^\d{5,15}$/.test(String(value || ""))
}
...
function syncToJsonFile (appid, groupOpenid, groupUin) {
  try {
    ...
    for (const mappedOpenid of Object.keys(raw[appid])) {
      if (mappedOpenid !== openid && String(raw[appid][mappedOpenid]) === uin) {
        delete raw[appid][mappedOpenid]
      }
    }

    raw[appid][openid] = uin
    writeGroupMap(raw)
  } catch (err) {
...
function getGroupOpenidFromFile (appid, groupUin) {
  const raw = readGroupMap()
  const appMap = raw[appid]
  if (!appMap || typeof appMap !== "object") return false

  const uin = String(groupUin)
  for (const openid of Object.keys(appMap)) {
    if (String(appMap[openid]) === uin) return openid
  }
  return false
}
 ...
 * @param {string} groupUin - 真实 QQ 群号（必须是合法的 5-15 位数字）
 * @returns {Promise<boolean>} 是否保存成功
 */
export async function saveGroupMapping (appid, groupOpenid, groupUin) {
  if (!appid || !groupOpenid || !isQQGroupUin(groupUin)) return false

  const uin = String(groupUin)
  const openid = String(groupOpenid)
  const oldUin = await getGroupMapping(appid, openid)
  const oldOpenid = await translateGroupToOpenid(appid, uin)

  // 双写：Redis（正向 + 反向键），并清理重绑产生的旧键
  if (global.redis) {
    try {
      if (oldUin && oldUin !== uin) {
        await redis.del(getGroupUinKey(appid, oldUin))
      }
      if (oldOpenid && oldOpenid !== openid) {
        await redis.del(getGroupKey(appid, oldOpenid))
      }
...
export async function translateGroupToOpenid (botOrId, groupUin) {
  if (!groupUin || !isQQGroupUin(groupUin)) return false
  ...
  try {
    return getGroupOpenidFromFile(appid, uin)
  } catch (err) {
