...
function getBinarySnapshot (value) {
  ...
  if (value?.type === "Buffer" && Array.isArray(value.data))
    return { type: "Buffer", byteLength: value.data.length }
}

function sanitizeRawValue (value, ancestors = new Set()) {
  const binary = getBinarySnapshot(value)
  ...
  try {
    ...
    if (jsonValue !== value) return sanitizeRawValue(jsonValue, ancestors)

    if (Array.isArray(value)) return value
      .filter(item => typeof item !== "string" || !/^auth_?token=/i.test(item))
      .map(item => sanitizeRawValue(item, ancestors))

    const result = {}
    for (const [key, item] of Object.entries(value)) {
      if (/^auth_?token$/i.test(key)) continue
      result[key] = sanitizeRawValue(item, ancestors)
    }
    return result
  } finally {
...
function cloneRawValue (value) {
  ...
  const json = JSON.stringify(sanitizeRawValue(value))
  return json === undefined ? undefined : JSON.parse(json)
}

/** 生成可安全序列化的消息快照，移除鉴权信息，并将二进制内容缩减为大小信息。 */
...
function getRawMessageId (raw) {
  /** QQ 官方事件体 id 是真实消息 ID；message_id 仅作为旧格式兼容回退。 */
  return firstDefined(raw?.id, raw?.message_id, raw?.messageid)
}
...
async function normalizeSegments (raw, appid, messageType) {
  ...
  for (const segment of Array.isArray(raw?.message) ? raw.message : []) {
    ...
    if (item.type === "at" && messageType === "group") {
      const rawUserId = String(firstDefined(item.user_openid, item._raw_user_id, item.user_id, item.userid, item.qq) || "")
      item._raw_user_id = rawUserId
      item.qq = rawUserId === "all" ? "all" : await getMappedUin(appid, rawUserId) || rawUserId
      item.user_id = item.qq
      delete item.user_openid
      delete item.userid
    }
    message.push(item)
  }
...
async function adaptJsonlRecord (adapter, bot, record) {
  const messageType = record.scene === "private" ? "private" : "group"
  const senderOpenId = String(record.sender?.user_openid || "")
  const conversationUserOpenId = String(record.user_openid || "")
  const rawUserId = record.direction === "outgoing"
    ? senderOpenId || String(record.self_id || bot?.uin || "")
    : senderOpenId || conversationUserOpenId
  const appid = getAppid(bot || record.self_id)
  const mappedUserId = rawUserId && record.direction !== "outgoing"
    ? await getMappedUin(appid, rawUserId)
    : false
  const rawGroupId = String(record.group_openid || "")
  const mappedGroupId = rawGroupId
    ? await getGroupUin(bot || record.self_id, rawGroupId)
    : false
  const userId = record.direction === "outgoing"
    ? String(record.self_id || bot?.uin || rawUserId)
    : mappedUserId || rawUserId
  const role = record.sender?.role || record.sender?.member_role
  const message = await normalizeSegments({ message: record.message }, appid, messageType)
  const fallbackRaw = {
    ...
    ...(rawGroupId ? { group_id: rawGroupId } : {}),
  }
  const raw = sanitizeRawMessage(record.raw || record.raw_response) || fallbackRaw
  const data = {
    raw,
    self_id: record.self_id || bot?.uin,
    post_type: "message",
    ...
    message_seq: record.msg_idx,
    msg_idx: record.msg_idx,
    time: Number(record.time) || Math.floor(Date.now() / 1000),
    user_id: userId,
    _raw_user_id: rawUserId,
    group_id: rawGroupId ? mappedGroupId || rawGroupId : undefined,
    _raw_group_id: rawGroupId || undefined,
    sender: {
      ...record.sender,
      user_id: userId,
      ...
      role,
      member_role: role,
      bot: record.direction === "outgoing" || record.sender?.bot === true,
      is_owner: role === "owner",
      is_admin: role === "owner" || role === "admin",
    },
    message,
  ...
  delete data.sender.user_openid
  const runtime = makeRuntimeContext(adapter, bot, data)
  if (record.raw_request !== undefined)
    Object.defineProperty(runtime, "raw_request", {
      value: sanitizeRawMessage(record.raw_request),
      configurable: true,
    })
  if (record.raw_response !== undefined)
    Object.defineProperty(runtime, "raw_response", {
      value: sanitizeRawMessage(record.raw_response),
      configurable: true,
    })
  return runtime
}
...
/** 将持久化消息记录恢复为插件可直接使用的框架消息对象。 */
export async function adaptMessageRecord (adapter, botOrId, record) {
  if (!record) return null
  const bot = typeof botOrId === "object" ? botOrId : Bot?.[botOrId]
  if (record.type === "message" && (record.group_openid || record.user_openid))
    return adaptJsonlRecord(adapter, bot, record)
