...
export async function getMappedOpenid (appid, uin) {
  ...
  try {
    // 优先精确查询：appid + uin
    const appRaw = await redis.get(getAppUinKey(appidStr, uin))
    if (appRaw) {
      const parsed = JSON.parse(appRaw)
      if (parsed?.openid) return parsed.openid
    }

    // 兜底：uin 映射表（多 bot 兼容，{ appid: openid, ... }）
    const raw = await redis.get(getUinKey(uin))
    if (!raw) return false
    const map = JSON.parse(raw)
    if (map && typeof map === "object" && map[appidStr]) {
      return map[appidStr]
    }
    return false
  } catch (err) {
    Bot.makeLog?.("error", ["QQBot 映射反查失败", appidStr, uin, err])
    return false
  }
}
...
export async function saveMapping (appid, openid, uin) {
  ...
  // openid -> uin（按 appid 隔离）
  await redis.set(getOpenidKey(appidStr, openidStr), uinStr)

  // appid + uin -> openid（精确反查，多 bot 不会冲突）
  const pair = JSON.stringify({ appid: appidStr, openid: openidStr })
  await redis.set(getAppUinKey(appidStr, uinStr), pair)

  // uin -> { appid: openid, ... }（支持多 bot，追加不覆盖）
  try {
    const existing = await redis.get(getUinKey(uinStr))
    const map = existing ? JSON.parse(existing) : {}
    map[appidStr] = openidStr
    await redis.set(getUinKey(uinStr), JSON.stringify(map))
  } catch {
    await redis.set(getUinKey(uinStr), JSON.stringify({ [appidStr]: openidStr }))
  }

  await syncToJsonFile(appidStr, openidStr, uinStr)
  return true
}
...
export async function applyUinMapping (data, openid) {
  ...
  // 映射后主动校验 master 权限，确保框架能识别主人
  try {
    const selfId = String(data.self_id || "")
    const userId = String(realUin)
    // 兼容 cfg.master（按 bot_id 分组）和 cfg.masterQQ（纯 QQ 号列表）
    const masterByBot = cfg?.master?.[selfId]
    const masterQQList = cfg?.masterQQ
    if (
      (masterByBot && masterByBot.includes(userId)) ||
      (masterQQList && masterQQList.map(i => String(i)).includes(userId))
    ) {
      data.isMaster = true
      Bot.makeLog?.("debug", ["QQBot 映射后识别为主人", selfId, openid, "->", realUin])
    }
  } catch (_) { /* 不阻塞消息流程 */ }
