  ...
  const makeMessage = async (id, event) => {
    ...
    if (Array.isArray(event.mentions)) {
      ...
      for (const m of event.mentions) {
        const id = m.id || m.member_openid
        if (!id || existingIds.has(id) || m.is_you) continue
        const atSeg = { type: "at", qq: id, user_id: id, username: m.username }
        if (data.message_type !== "group") atSeg.qq = `qg_${id}`
