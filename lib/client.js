...
import { applyUinMapping, handleBindMessage } from "./uinMap.js"
export async function connectBot (adapter, token) {
  ...
  const makeFriendMessage = async (data, event) => {
    ...
    const replyData = { ...data, user_id: rawOpenid }
    if (await handleBindMessage(data, rawOpenid, adapter, replyData)) return true
