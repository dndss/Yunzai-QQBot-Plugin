...
export async function connectBot (adapter, token) {
  ...
  // 确保 cfg.master 中有该 bot 的 self_id 条目，避免 master 权限失效
  try {
    if (cfg?.masterQQ?.length && !cfg.master?.[id]) {
      cfg.master[id] = cfg.masterQQ.map(i => String(i))
      Bot.makeLog?.("info", [`QQBot 自动补充 master 列表：${id} -> [${cfg.master[id].join(",")}]`])
    }
  } catch (_) { /* 不阻塞连接 */ }
  ...
  const makeFriendMessage = async (data, event) => {
    ...
    if (await handleBindMessage(data, rawOpenid, adapter, replyData)) return
