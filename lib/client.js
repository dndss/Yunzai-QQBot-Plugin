...
export async function connectBot (adapter, token) {
  ...
  const makeCallback = async (id, event) => {
    ...
    if (callback) {
      ...
      if (callback.type === "image-retry") {
        data.raw_message = "[重新发送图片]"
      } else {
    ...
    if (callback?.type === "image-retry") {
      const msg = adapter.converter.makeImageRetryMsg(callback.images)
      if (msg) await data.reply(msg)
      else Bot.makeLog("warn", "图片重发缓存已失效", data.self_id)
      return
    }
