...
import { parseSceneExt, parseElementMessage, getElementBrief, saveMsgRecord, getRecordByMsgIdx } from "./messageStore.js"
...
async function makeFriendMessage(adapter, data, event) {
  ...
  await saveIncomingRecord(data, event, "private")
  return false
}
...
/** 持久化群消息，并为带引用的消息挂载 e.source / e.getReply() */
async function attachGroupReplyContext (data, event) {
  const { msg_idx, ref_msg_idx } = parseSceneExt(event)
  await saveIncomingRecord(data, event, "group")

  if (!ref_msg_idx) return
  const stored = await getRecordByMsgIdx(data.bot || data.self_id, ref_msg_idx, {
