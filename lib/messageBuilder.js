...
import { parseSceneExt, parseElementContent, saveMsgRecord, getRecordByMsgIdx } from "./msgIdxCache.js"
...
async function makeGroupMessage(adapter, data, event) {
  ...
  await attachGroupReplyContext(data, event)
  await adapter.setGroupMap(data)
}

/** 存储群消息记录，并为带引用的消息挂载 e.source / e.getReply() */
async function attachGroupReplyContext (data, event) {
  ...
  if (msg_idx)
    await saveMsgRecord(data.bot || data.self_id, {
      ...
      _raw_user_id: data._raw_user_id,
      group_id: data.group_id,
      _raw_group_id: data._raw_group_id,
      time: event.timestamp || Math.floor(Date.now() / 1000),
      sender: { ...data.sender },
      message: data.message,
      raw_message: data.raw_message,
      raw: typeof event.toJSON === "function" ? event.toJSON() : undefined,
    })

  if (!ref_msg_idx) return
  const cached = await getRecordByMsgIdx(data.bot || data.self_id, ref_msg_idx)
  if (cached) {
    data.source = {
      user_id: cached.user_id,
      time: cached.time,
      /** QQBot 无消息序号，以 message_id 顶替，配合 group.getChatHistory 按此查缓存 */
      seq: cached.message_id,
      rand: 1,
      message: cached.raw_message,
    }
    data.getReply = async () => cached
    return
  }

  /** 缓存未命中（如引用了 Bot 自己或上线前的消息）：用事件自带的引用内容预览兜底，无 message_id 不可撤回 */
  ...
  const fallback = {
    message: parseElementContent(el.content),
    raw_message: el.content,
    sender: {},
