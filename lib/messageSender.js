...
import { deleteMsgRecord } from "./msgIdxCache.js"
...
export function installMessageSender(adapter) {
  const sendMsgs = async (data, send, fullMsgs) => {
    const rets = { message_id: [], data: [], error: [] }

    for (const full of fullMsgs) {
      const hasReply = Array.isArray(full) && full[0]?.type === "reply"
      const replyKey = hasReply ? getReplyKey(data.self_id, full[0]) : undefined
      let i = full
      ...
      try {
        ...
        if (usedReply) {
          ...
          try {
            const ret = await send(full.slice(1))
            Bot.makeLog("debug", ["发送消息返回", ret], data.self_id)
            rets.data.push(ret)
            if (ret.id) rets.message_id.push(ret.id)
            continue
  ...
  adapter.sendMsg = async (data, send, msg) => {
    const rets = await sendMsgs(data, send, await adapter.converter.makeRawMarkdownMsg(data, msg))
