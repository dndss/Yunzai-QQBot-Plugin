...
export function installMessageSender(adapter) {
  adapter.sendMsg = async (data, send, msg) => {
    ...
    const sendMsgIter = async (replyFallback = false) => {
      for (const i of msgs)
        try {
          ...
          if (replyFallback) Bot.makeLog("warn", "主动发送失败正在以回复重发", data.self_id)
          else Bot.makeLog("error", ["发送消息错误", i, err], data.self_id)
          rets.error.push(err)
          return false
        }
    }
    ...
    const hasReplyFallback = fullMsgs.some(m => Array.isArray(m) && m[0]?.type === "reply")
    let msgs = fullMsgs.map(m => {
      if (Array.isArray(m) && m[0]?.type === "reply") return m.slice(1)
      return m
    })

    if ((await sendMsgIter(hasReplyFallback)) === false) {
      msgs = fullMsgs
      await sendMsgIter()
    }
