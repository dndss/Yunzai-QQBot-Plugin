...
export class Converter {
  ...
  async makeImageRetryButtonMsgs (data, messages, carrier, images, mergeWithImage) {
    ...
    if (mergeWithImage) {
      const markdown = messages.find(message => Array.isArray(message) && message[0]?.type === "markdown")
      if (markdown) {
  ...
  isSingleImageMsg (msg) {
    if (Array.isArray(msg)) return msg.length === 1 && msg[0]?.type === "image"
  ...
  async makeRawMarkdownMsg (data, msg, nested, retryImages = []) {
    ...
    const mergeRetryButton = !nested && this.isSingleImageMsg(msg)
    const messages = [],
      button = []
    let content = "",
      reply
