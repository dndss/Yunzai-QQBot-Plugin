...
import { getRecordByMsgId, parseSceneExt } from "./messageStore.js"
...
export class Converter {
  constructor (adapter) {
    ...
    this.markdownImageRetries = new WeakMap()
  }
  ...
  async makeMarkdownImage (data, file, summary = "图片") {
    ...
    return {
      ...
      url: `(${image.url})`,
      file: Buffer.isBuffer(buffer) ? buffer : undefined,
    }
  }

  async makeReply (reply, data, quote = false) {
    ...
    if (!quote || !reply.data?.id || reply.data.msg_idx) return reply

    let msgIdx
    if (reply.data.id === data.message_id)
      msgIdx = data.msg_idx || data.raw?.msg_idx || parseSceneExt(data.raw).msg_idx
    else {
      const scope = data._raw_group_id
        ? { groupOpenId: data._raw_group_id }
        : data._raw_user_id
          ? { userOpenId: data._raw_user_id }
          : undefined
      msgIdx = (await getRecordByMsgId(data.bot || data.self_id, reply.data.id, scope))?.msg_idx
    }

    if (msgIdx)
      return { ...reply, data: { ...reply.data, msg_idx: msgIdx } }
    return reply
  }
    ...
    if (button.permission && data.group_id) {
      if (button.permission === "admin") {
        ...
        if (permission === "master")
          permission = cfg.master[String(data.self_id)] || []

        msg.action.permission.type = 0
        msg.action.permission.specify_user_ids = []
        if (!Array.isArray(permission)) permission = [permission]
        for (const id of permission) {
          const stripped = String(id).replace(`${data.self_id}${this.adapter.sep}`, "")
  ...
  // 使用缓存的图片 Buffer 刷新指定 Markdown 消息中的图片 URL。
  async refreshMarkdownImages (data, message) {
    const images = this.markdownImageRetries.get(message)
    if (!images?.length) return

    const markdownIndex = message.findIndex(segment => segment?.type === "markdown")
    if (markdownIndex < 0 || typeof message[markdownIndex].data?.content !== "string") return

    let content = message[markdownIndex].data.content
    const refreshedImages = []
    for (const image of images) {
      if (!content.includes(image.markdown)) return
      const refreshed = await this.makeMarkdownImage(data, image.file, image.summary)
      const markdown = `${refreshed.des}${refreshed.url}`
      content = content.replace(image.markdown, markdown)
      refreshedImages.push({
        file: refreshed.file || image.file,
        summary: image.summary,
        markdown,
      })
    }

    const refreshedMessage = message.map((segment, index) => index === markdownIndex
      ? { ...segment, data: { ...segment.data, content } }
      : segment)
    this.markdownImageRetries.set(refreshedMessage, refreshedImages)
  ...
  async makeRawMarkdownMsg (data, msg, nested) {
      ...
      switch (i.type) {
        ...
        case "image": {
          if (this.adapter.config.markdownImage === true) {
            const { des, url, file } = await this.makeMarkdownImage(data, i.data.file, i.data.summary)
            const markdown = `${des}${url}`
            content += markdown
            if (file) contentImages.push({ file, summary: i.data.summary, markdown })
        ...
        case "markdown":
          if (typeof i.data.data === "object") {
            const markdown = { ...i.data.data }
            if (this.adapter.config.forceVerifyImageResource === true)
              markdown.force_verify_image_resource = true
            else
              delete markdown.force_verify_image_resource
            messages.push([{ type: "markdown", data: markdown }])
          }
    ...
    if (content) {
      const markdown = {
        ...
        data: {
          content,
          ...(this.adapter.config.forceVerifyImageResource === true
            ? { force_verify_image_resource: true }
            : {}),
        },
      }
      const markdownMessage = [markdown]
      messages.unshift(markdownMessage)
      if (contentImages.length && this.adapter.config.forceVerifyImageResource === true)
        this.markdownImageRetries.set(markdownMessage, contentImages)
    }
    ...
    for (let i of Array.isArray(msg) ? msg : [msg]) {
      ...
      switch (i.type) {
        ...
        case "markdown":
          ...
          delete i.data.force_verify_image_resource
          break
        case "button":
          continue
        case "node":
