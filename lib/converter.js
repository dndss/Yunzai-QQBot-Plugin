...
export class Converter {
  ...
  async makeButton (data, button, style) {
    ...
    if (button.input)
    ...
    else if (button.QQBot?.imageRetry) {
      msg.action = {
        type: 1,
        permission: { type: 2 },
        ...button.QQBot?.action,
      ...
      data.bot.callback[msg.id] = {
        ...
        images: button.QQBot.imageRetry,
      }
  ...
  // 为自动转换的 Markdown 图片缓存 Buffer 并附加内部重发按钮。
  async makeImageRetryButtonMsgs (data, messages, carrier, images) {
    if (this.adapter.config.markdownImage !== true || this.adapter.config.imageRetryButton === false) return
    if (!images.length || !data.user_id) return
    const hasButton = messages.some(message =>
      (Array.isArray(message) ? message : [message]).some(segment => segment?.type === "button"),
    )
    const button = await this.makeButtons(data, [{
      text: "图片加载失败？点此重发",
      clicked_text: "已重新发送",
      permission: data.user_id,
      QQBot: { imageRetry: images },
    }])
    if (!button.length) return
    if (!hasButton) {
      const markdown = [...messages].reverse().find(
        message => Array.isArray(message) && message[0]?.type === "markdown",
      )
      if (markdown) {
    ...
    messages.push([carrier, ...button])
  }

  // 将缓存恢复成图片消息段，重新进入转换流程以生成新 URL 和新重发按钮。
  makeImageRetryMsg (images) {
    const msg = images
      .filter(image => Buffer.isBuffer(image.file))
      .map(image => ({ type: "image", file: image.file, summary: image.summary }))
  ...
  // Raw Markdown 消息组装
  async makeRawMarkdownMsg (data, msg, nested, retryImages = []) {
      ...
      switch (i.type) {
        ...
        case "image": {
          if (this.adapter.config.markdownImage === true) {
            ...
            content += `${des}${url}`
            if (file) retryImages.push({ file, summary: i.data.summary })
        ...
        case "node":
          for (const { message } of i.data.data)
            messages.push(...(await this.makeRawMarkdownMsg(data, message, true, retryImages)))
          continue
        case "raw":
          messages.push(Array.isArray(i.data.data) ? i.data.data : [i.data.data])
          break
    ...
    if (!nested)
      await this.makeImageRetryButtonMsgs(
        data,
        messages,
