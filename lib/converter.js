...
export class Converter {
  ...
  // 提取触发本次回复的原始命令文本
  getTriggerCommand (data) {
    if (Array.isArray(data.message)) {
      let text = ""
      for (const i of data.message)
        if (i.type === "text" && i.text) text += i.text
      text = text.trim()
      if (text) return text
    }
    const raw = typeof data.raw_message === "string" ? data.raw_message : ""
    return raw.replace(/\[回复：[^\]]*\]/g, "").trim()
  }

  // 检测已构建消息中是否包含图片（原生图片段或 Markdown 图片）
  hasImageInMessages (messages) {
    for (const message of messages)
      for (const seg of Array.isArray(message) ? message : [message]) {
        if (seg?.type === "image") return true
        if (seg?.type === "markdown" && /!\[[^\]]*\]\([^)]+\)/.test(seg.data?.content || "")) return true
      }
    return false
  }

  // 图片消息附加重发按钮：Markdown 图片可能加载失败，追加一条按钮消息，
  // 仅触发者可点击，点击后重新执行原命令以重发图片
  async makeImageRetryButtonMsgs (data, messages, carrier) {
    if (this.adapter.config.imageRetryButton === false) return
    if (!this.hasImageInMessages(messages)) return
    const command = this.getTriggerCommand(data)
    if (!command || !data.user_id) return
    ...
    if (button.length) messages.push([carrier, ...button])
  }
  ...
  async makeNativeRichMsg (data, msg, nested) {
    ...
    if (!nested) await this.makeImageRetryButtonMsgs(data, messages, { type: "text", data: { text: " " } })
  ...
  async makeRawMarkdownMsg (data, msg, nested) {
    ...
    if (!nested) await this.makeImageRetryButtonMsgs(data, messages, { type: "markdown", data: { content: " " } })
