...
export class Converter {
  ...
  makeReply (reply, data) {
    ...
    if (reply.id?.startsWith?.("event_")) {
      if (data?.raw?.event_id) return { type: "reply", event_id: data.raw.event_id }
      return false
    }
    if (reply.id?.startsWith?.("INTERACTION_CREATE:")) {
      const event_id = reply.id.replace(/^INTERACTION_CREATE:/, "")
      if (data?.raw?.event_id === event_id) return { type: "reply", event_id }
      return false
    }
    return reply
  }
    ...
    for (let i of Array.isArray(msg) ? msg : [msg]) {
      ...
      switch (i.type) {
        ...
        case "file":
          ...
          content += this.makeRawMarkdownText(`文件：${i.file}`)
          break
        case "at":
          ...
          break
        case "text":
          content += this.makeRawMarkdownText(i.text)
          break
        case "image": {
        ...
        case "markdown":
          ...
          break
        case "button":
          button.push(...await this.makeButtons(data, i.data))
          break
        case "reply":
          reply = i
          continue
