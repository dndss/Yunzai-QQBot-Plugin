...
function getMessageRetentionDays (botOrId) {
  const bot = typeof botOrId === "object"
    ? botOrId
    : globalThis.Bot?.[getBotId(botOrId)]
  const days = Number(bot?.adapter?.config?.messageRetentionDays)
  return Number.isInteger(days) && days > 0 ? days : 0
}

function getRetentionCutoffName (retentionDays, now) {
...
async function cleanupExpiredJsonlFiles (directory, retentionDays, now = Date.now()) {
  const currentDate = formatDate(now)
  if (cleanupTokenDate !== currentDate) {
    cleanupTokens.clear()
    cleanupTokenDate = currentDate
  }
  const token = `${currentDate}:${retentionDays}`
  if (cleanupTokens.get(directory) === token) return 0

  const cutoffName = getRetentionCutoffName(retentionDays, now)
  const entries = await fs.readdir(directory, { withFileTypes: true })
  let removed = 0

  for (const entry of entries) {
    if (!entry.isFile() || !/^\d{4}-\d{2}-\d{2}\.jsonl$/.test(entry.name)) continue
    if (entry.name.slice(0, 10) >= cutoffName) continue
    await fs.unlink(path.join(directory, entry.name))
    removed++
  }

  cleanupTokens.set(directory, token)
  return removed
}
...
function enqueueAppend (filePath, line, retentionDays, botId) {
  ...
  const current = previous
    ...
    .then(async () => {
      ...
      if (retentionDays > 0) {
        try {
          const removed = await cleanupExpiredJsonlFiles(directory, retentionDays, now)
          if (removed > 0) globalThis.Bot?.makeLog?.("info", [
            `QQBot 已清理过期 JSONL 消息记录，共 ${removed} 个文件`,
            `retentionDays=${retentionDays}`,
          ], botId)
        } catch (err) {
          globalThis.Bot?.makeLog?.("error", [
            "QQBot 过期 JSONL 消息记录清理失败",
            directory,
            err,
          ], botId)
        }
        if (path.basename(filePath, ".jsonl") < getRetentionCutoffName(retentionDays, now)) return
      }
...
export async function saveMsgRecord (botOrId, record) {
  ...
  try {
    ...
    await enqueueAppend(
      ...
      getMessageRetentionDays(botOrId),
      normalized.self_id,
    )
    return true
  } catch (err) {
...
function parseReferenceAttachment (source) {
  ...
  for (let index = 0; index < matches.length; index++) {
    ...
    fields[match[1]] = source.slice(start, end).trim()
  }

  const rawType = fields["类型"]
  if (!rawType) return null
  const name = fields["文件名"] || rawType
  const url = fields.URL
  if (!url) return { type: "text", text: `[${rawType}：${name}${fields["大小"] ? `，大小：${fields["大小"]}` : ""}]` }

  let type = "file"
  if (rawType === "动图" || rawType.includes("图片")) type = "image"
  else if (rawType.includes("视频")) type = "video"
  else if (rawType.includes("语音") || rawType.includes("音频")) type = "record"
...
/** 将 QQBot message_type=103 的引用预览还原成标准合并消息节点。 */
function parseReferenceForwardContent (content) {
  if (typeof content !== "string" || !content) return null
  const normalized = content.replace(/\r\n/g, "\n")
  const marker = normalized.match(/^[ \t]*\[消息类型\][ \t]*引用消息[ \t]*\n[ \t]*\[关联消息\][ \t]*\n/m)
  if (!marker) return null

  const related = normalized.slice((marker.index || 0) + marker[0].length)
  const sectionPattern = /^[ \t]*--- 第(\d+)条 ---[ \t]*$/gm
  const sections = [...related.matchAll(sectionPattern)]
  if (!sections.length) return null

  const nodes = []
  for (let index = 0; index < sections.length; index++) {
    const start = (sections[index].index || 0) + sections[index][0].length
    const end = sections[index + 1]?.index ?? related.length
    const lines = related.slice(start, end).split("\n").map(line => line.trim())
    const message = []
    const textLines = []
    let nickname = "匿名消息"
    ...
    for (const line of lines) {
      if (!line) continue
      const sender = line.match(/^\[发送者\][ \t]*(.*)$/)
      if (sender) {
        if (sender[1].trim()) nickname = sender[1].trim()
        continue
      }
      const attachment = line.match(/^\[附件\d+\][ \t]*(.*)$/)
      if (attachment) {
        const segment = parseReferenceAttachment(attachment[1])
        if (segment) message.push(segment)
        else textLines.push(line)
        continue
      }
      textLines.push(line.replace(/^\[消息内容\][ \t]?/, ""))
    }

    const text = textLines.join("\n").trim()
    if (text) message.unshift({ type: "text", text })
    if (message.length) nodes.push({ nickname, message })
  }

  if (!nodes.length) return null
  const title = normalized.slice(0, marker.index || 0).match(/^\[消息内容\][ \t]*(.+)$/m)?.[1]?.trim()
  return { type: "node", ...(title ? { title } : {}), data: nodes }
}

/** 将 msg_elements 的正文和附件统一解析为 Yunzai 消息段。 */
export function parseElementMessage (element, options = {}) {
  const referenceForward = options.parseReferenceForward === true
    ? parseReferenceForwardContent(element?.content)
    : null
  const message = referenceForward ? [referenceForward] : parseElementContent(element?.content)
